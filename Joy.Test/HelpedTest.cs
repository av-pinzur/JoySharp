...
namespace AvP.Joy.Test
{
    ...
    {
        protected void AssertThrows(Type expectedExceptionType, Action action, string? message = null, params object[] formatArgs)
        {
            AssertThrows(expectedExceptionType, AsVoidish(action), message, formatArgs);
        }

        protected void AssertThrows(Type expectedExceptionType, string? expectedExceptionMessage, Action action, string? message = null, params object[] formatArgs)
        {
            AssertThrows(expectedExceptionType, expectedExceptionMessage, AsVoidish(action), message, formatArgs);
        }

        protected void AssertThrows<TResult>(Type expectedExceptionType, Func<TResult> function, string? message = null, params object[] formatArgs)
        {
        ...
        private Tuple<TResult?, Exception?> CatchingAll<TResult>(Func<TResult> function)
        {
            ...
            {
                return new Tuple<TResult?, Exception?>(default, e);
            }
        }
